...
class TokenInfoNCEMax(nn.Module):
    def __init__(self, T=0.1):
        super(TokenInfoNCE, self).__init__()
        self.T = T

    def forward(self, global_feat, part_feat, labels, reduce='max'):
        ...
        dist = dist.view(B, PART_NUM, B, PART_NUM)[:, 0].max(-1)[0]
    
        mask = torch.zeros((B, B)).to(global_feat.device)
        ...
        pos_dist = dist[pos_mask_tmp].view(B, -1).max(-1)[0] / self.T
